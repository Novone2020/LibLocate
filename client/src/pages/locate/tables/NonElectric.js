...
export const NonElectric = () => {
	...
	const autofetch = () => {
		axios
			.get(`${process.env.REACT_APP_baseurl}/pict0/nonElecTable`)
			.then((res) => {
				setData(res.data);
			})
			.catch((err) => {
		...
		axios
			.get(`${process.env.REACT_APP_baseurl}/pict0/smallWall`)
			.then((res) => {
				setWall(res.data);
			})
			.catch((err) => {
	...
	const updateTable = async (id, section) => {
		...
		await axios
			.patch(
				`${process.env.REACT_APP_baseurl}/pict0/nonElecTable/${section}/${id}`,
	...
	const updateSmallWall = async (id) => {
		await axios
			.patch(`${process.env.REACT_APP_baseurl}/pict0/smallWall/${id}`, {
				user: localStorage.userID,
			})
			.then((res) => {
				if (res.data.done) autofetch();
