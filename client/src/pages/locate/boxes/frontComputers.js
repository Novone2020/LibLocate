...
export const FrontComputers = () => {
	...
	const autofetch = () => {
		...
		axios
			.get(`${process.env.REACT_APP_baseurl}/pict0/frontComputers`)
			.then((res) => {
				setData(res.data);
		...
		axios
			.get(`${process.env.REACT_APP_baseurl}/pict0/frontWall`)
			.then((res) => {
				SetWall(res.data);
				// console.log(res.data);
			})
	...
	const updateField = async (id) => {
		await axios
			.patch(
				`${process.env.REACT_APP_baseurl}/pict0/frontComputers/${id}`,
	...
	const updateWall = async (id) => {
		await axios
			.patch(`${process.env.REACT_APP_baseurl}/pict0/frontWall/${id}`, {
				user: localStorage.userID,
			})
			.then((res) => {
				if (res.data.done) autofetch();
