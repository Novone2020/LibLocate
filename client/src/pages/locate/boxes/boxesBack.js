...
export const BoxesBack = () => {
	...
	const autofetch = () => {
		axios
			.get(`${process.env.REACT_APP_baseurl}/pict0/backboxes`)
			.then((res) => {
				setData(res.data);
			})
			.catch((err) => {
		...
		axios
			.get(`${process.env.REACT_APP_baseurl}/pict0/backBoxesTables`)
			.then((res) => {
				setTable(res.data);
	...
	const updateField = async (id) => {
		await axios
			.patch(`${process.env.REACT_APP_baseurl}/pict0/backboxes/${id}`, {
	...
	const updateRight = async (id) => {
		await axios
			.patch(
				`${process.env.REACT_APP_baseurl}/pict0/backBoxesTables/right/${id}`,
				{ user: localStorage.userID }
			)
			.then((res) => {
				if (res.data.done) autofetch();
	...
	const updateLeft = async (id) => {
		await axios
			.patch(
				`${process.env.REACT_APP_baseurl}/pict0/backBoxesTables/left/${id}`,
				{ user: localStorage.userID }
			)
			.then((res) => {
				if (res.data.done) autofetch();
	...
	const updateTop = async (id) => {
		await axios
			.patch(
				`${process.env.REACT_APP_baseurl}/pict0/backBoxesTables/top/${id}`,
				{ user: localStorage.userID }
			)
			.then((res) => {
				if (res.data.done) autofetch();
	...
	const updateBottom = async (id) => {
		await axios
			.patch(
				`${process.env.REACT_APP_baseurl}/pict0/backBoxesTables/bottom/${id}`,
				{ user: localStorage.userID }
			)
			.then((res) => {
				if (res.data.done) autofetch();
