...
export const Boxes = () => {
  ...
  const autofetch = () => {
    axios
      .get("https://liblocate-server.onrender.com/pict0/boxes")
      .then((res) => {
        setData(res.data);
      });
  };
  ...
  const updateRight = async (id, ind) => {
    console.log(id, ind);
    await axios
      .patch(`https://liblocate-server.onrender.com/pict0/boxes/right/${id}`, {
  ...
  const updateLeft = async (id, ind) => {
    console.log(id, ind);
    await axios
      .patch(`https://liblocate-server.onrender.com/pict0/boxes/left/${id}`, {
        user: localStorage.userID,
        ind,
      })
      .then((res) => {
                  ...
                  <div className="boxex-row-up">
                    <>
                      {info.right.map((val, ind) => {
                        return (
                          <button
                            className={`boxex-buty ${
                              val.state === 1 ? "green-button" : "white-button"
                            }`}
                            key={ind}
                            title={val.user}
                            onClick={() => {
                              updateRight(info._id, ind);
                            }}
                          >
                            🪑
                          </button>
                      ...
                      <center>
                        <div
                          style={{ border: "solid 1px white", width: "100%" }}
                        ></div>
                      </center>
